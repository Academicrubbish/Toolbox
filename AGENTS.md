...
详细规范见 `.Codex/rules/` 目录：
- [code-style.md](.Codex/rules/code-style.md) — 代码风格（ES6+、Vue 组件结构、API 层、样式、分包）
- [commit-convention.md](.Codex/rules/commit-convention.md) — Commit 格式（`<type>(<scope>): <subject>`，subject 用中文）
...
详见 [ai-product-principles.md](.Codex/rules/ai-product-principles.md) — 产品定位、长文本差异化策略、AI 功能不可替代性检验标准、已识别用户场景。
