	...
	<view>
		...
		<view class="cu-list menu sm-border">
			...
			<view class="cu-item arrow" @tap="goDictCategory">
				...
				<view class="action">
					<view class="cu-tag round bg-orange light">{{dictTop[0].name}}</view>
					<view class="cu-tag round bg-olive light">{{dictTop[1].name}}</view>
					<view class="cu-tag round bg-blue light">{{dictTop[2].name}}</view>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		onLoad() {
			// 获取标签top3展示
			getDictTopList().then(res => {
				this.dictTop = res.result.data
			})
		},
		methods: {
			goDepart() {
